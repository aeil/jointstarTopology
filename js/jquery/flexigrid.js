...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		var g = {
			...
			addData: function (data) { //parse data
				...
				console.log(p.page, p.newp, p.pages);
				this.buildpager();
				//build new body
			...
			changePage: function (ctype) { //change page
				...
				switch (ctype) {
					case 'first':
						...
						break;
					case 'prev':
					console.log("before: p.newp", p.newp, "p.page=", p.page);
						if (p.page > 1) {
							p.newp = parseInt(p.page, 10) - 1;
						}
					console.log("after: p.newp", p.newp, "p.page=", p.page);
						break;
					case 'next':
					console.log("before: p.newp", p.newp, "p.page=", p.page);
						if (p.page < p.pages) {
				...
				*/
				if (p.onChangePage) {
					console.log("in onChangePage, p.newp=",
					p.newp, ", p=", p);
					if (!p.newp) {
						p.newp = 1;
					}
	...
	$.fn.flexbuildpager = function(p) { // function to search grid
	return this.each( function() { this.grid.buildpager(); });
}; //end flexSearch
	...
	$.fn.flexGetOptions=function(){var data={};this.each    (function(){data=this.grid.getoptions();}); return data; };
})(jQuery);
